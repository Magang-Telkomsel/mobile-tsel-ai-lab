...
class _MessageInputIntegratedState extends State<MessageInputIntegrated> {
  ...
  bool _supportsFileUpload(String modelName) {
    return modelName == 'TSEL-PDF-Agent' ||
        modelName == 'TSEL-Lerning-Based' ||
        modelName == 'TSEL-Learning-Based'; // Support both variants
  }

  // Get appropriate hint text based on model
  String _getHintText(String modelName, bool supportsFileUpload) {
    if (modelName == 'TSEL-Lerning-Based' ||
        modelName == 'TSEL-Learning-Based') {
  ...
  String _getShortModelName(String modelName) {
    // Debug logging untuk melihat model name yang diterima
    print(
      'Getting short name for model: "$modelName" (length: ${modelName.length})',
    );

    // Shortening model names for display
    switch (modelName.trim()) {
      // Tambahkan trim() untuk menghapus whitespace
      case 'TSEL-Chatbot':
        return 'Chatbot';
      case 'TSEL-Lerning-Based': // Model name dengan typo (sesuai dengan chatpage_screen.dart)
        return 'Learning Based';
      case 'TSEL-Learning-Based': // Tetap keep yang benar juga untuk compatibility
        return 'Learning Based';
      case 'TSEL-PDF-Agent':
        return 'PDF Agent';
      ...
      default:
        print(
          'Model name "$modelName" not found in switch cases, returning default "AI"',
        );
        return 'AI';
    }
  }
